...
* How does season, hour of the day, rainy or snowly conditions affect the response time?
...
wash_dat <- 
  dat_subset %>% 
  filter(neighborhood == 'Inwood and Washington Heights')
...
On UWS, high precipitation is associated with lower odds of response times greater than 8 minutes. Street closure is associated with the outcome but not statistically significant.
Washington Heights, on the other hand, saw afternoon and dawn time, compared to the night time as reference, as contributing factors to the response time greater than 8 minutes with ORs 1.6 and 1.3, respectively. 

```{r}
fit_logistic_neighbor =
   glm(over_8min ~ season + hour_of_day + snow_ctg + prcp_ctg + street_closed + neighborhood, family = binomial(), data = dat_subset)
...
* If all the response vehicles have the necessary equipments to deal with any emergency situation.
